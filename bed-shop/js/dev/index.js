...
function initSliders() {
  if (document.querySelector(".categories__tabs")) {
    const sliders = document.querySelectorAll(".categories__tabs .swiper");
    sliders.forEach((el) => {
      new Swiper(el, {
  ...
  if (document.querySelector(".blog__slider")) {
    new Swiper(".blog__slider", {
      ...
      */
      // Кнопки "вліво/вправо"
      navigation: {
        prevEl: ".blog__slidebar-button-prev",
        nextEl: ".blog__slidebar-button-next"
      },
      /*
      // Брейкпоінти
      breakpoints: {
...
window.addEventListener("load", function() {
  const buttonSelector = "button.profile-header__button";
  const selectSelector = '[data-fls-select-id="1"]';
  const select = document.querySelector(selectSelector);
  document.addEventListener("click", function(e) {
    if (e.target.closest(buttonSelector)) {
      select.classList.toggle("select--visible");
    }
    if (!e.target.closest(buttonSelector) && !e.target.closest(selectSelector)) {
      select.classList.contains("select--visible") ? select.classList.remove("select--visible") : null;
    }
  });
});
function formInit() {
