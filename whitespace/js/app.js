(() => {
    ...
    function circleText() {
        const text = document.querySelector("[data-circleText]");
        text.innerHTML = text.innerText.split("").map(((char, i) => `<span style='transform: rotate(${i * 15}deg)'>${char}</span>`)).join("");
    }
    function activeClass() {
        const tabs = document.querySelector(".info-header__tabs");
        const buttons = document.querySelectorAll(".info-header__button");
        tabs.addEventListener("click", (e => {
            if (e.target.classList.contains("info-header__button")) {
